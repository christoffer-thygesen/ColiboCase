...
public class BaseAuthentication {
	...
	public BaseAuthentication(List<String> scopes, String clientId, String authority, String redirectUri,
			NationalCloud nationalCloud, String tenant, String clientSecret) {
		this.scopes = scopes;
	...
	protected String getAccessTokenSilent() {
		...
		if (this.response == null || durationPassed < 0)
			return null;
		try {
			if (durationPassed >= response.getExpiresIn() * 1000) {
				TokenRequestBuilder token = OAuthClientRequest
						.tokenLocation(this.authority + AuthConstants.TOKEN_ENDPOINT).setClientId(this.clientId)
						.setScope(getScopesAsString())
						.setRefreshToken(response.getRefreshToken())
						.setGrantType(GrantType.REFRESH_TOKEN).setScope(getScopesAsString())
